...
export default function LiveTracker() {
  ...
  useEffect(() => {
    (async () => {
      try { const b = await window.storage.get("bets"); if (b) setBets(JSON.parse(b.value)); } catch (e) {}
      try { const br = await window.storage.get("bankroll"); if (br) setBankroll(JSON.parse(br.value)); } catch (e) {}
      try { const bd = await window.storage.get("brand"); if (bd) setBrand(bd.value); } catch (e) {}
      setLoaded(true);
    })();
  }, []);
  useEffect(() => { if (loaded) window.storage.set("bets", JSON.stringify(bets)).catch(() => {}); }, [bets, loaded]);
  useEffect(() => { if (loaded) window.storage.set("bankroll", JSON.stringify(bankroll)).catch(() => {}); }, [bankroll, loaded]);
  useEffect(() => { if (loaded) window.storage.set("brand", brand).catch(() => {}); }, [brand, loaded]);
